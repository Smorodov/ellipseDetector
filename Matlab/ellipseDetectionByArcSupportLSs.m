...
% ellipses: N by 5. (center_x, center_y, a, b, phi)
    ...
    [candidates, edge, normals] = generateEllipseCandidates(I, 2, specified_polarity);%1,sobel; 2,canny
    t1 = clock;
    ...
    for i = 1 : size(list, 1)
        ...
        if (new_info == 1)
            if ( (((new_ellipse(1) - ellipseCenter(1))^2 + (new_ellipse(2) - ellipseCenter(2))^2 ) <= 16 * distance_tolerance_square) ...
                ...
                cosang=dot(normals(i_dx,:), ellipse_normals, 2);
                newinliers = i_dx(labels(i_dx) == 0 & (dRosin_square(new_ellipse,points(i_dx,:)) <= distance_tolerance_square & ((cosang*(-ellipse_polarity)) >= 0.923879532511287) ) );
        ...
        if (length(inliers) >= floor( pi * (1.5*sum(new_ellipse(3:4))-sqrt(new_ellipse(3)*new_ellipse(4))) * Tmin ))
            ...
            completeness=calcuCompleteness(points(inliers,:),new_ellipse(1:2),tbins);
            completeOrNot = completeness >= angleCoverage;
            if (new_info == 1 && completeOrNot )
                if (all( (sqrt(sum((ellipses(:, 1 : 2) - repmat(new_ellipse(1:2), size(ellipses, 1), 1)) .^ 2, 2)) > distance_tolerance) ...
                   ...
                   | (abs(ellipses(:, 5) - repmat(new_ellipse(5), size(ellipses, 1), 1)) >= 0.314159265358979 ) )) %0.1 * pi = 0.314159265358979 = 18бу

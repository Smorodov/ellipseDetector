...
// https://eigen.tuxfamily.org/dox/classEigen_1_1GeneralizedEigenSolver.html#a62f01cd78271efd5e39bcb24e0fe1a58
...
void groupLSs(double* lines, int line_num, int* region, int imgx, int imgy, std::vector<std::vector<int>>* groups)
{
    ...
    point2i* votebin = (point2i*)calloc(line_num, sizeof(point2i));
    int bincnt = 0;
    int xx, yy, temp;
...
void cvCanny3(cv::Mat& src, cv::Mat& dst,
    cv::Mat& dx, cv::Mat& dy,
    int aperture_size)
{
    //cv::Ptr<CvMat> dx, dy;
    ...
    cv::Sobel(src, dy, dy.type(), 0, 1, aperture_size);
...
void Canny3(cv::Mat& image, cv::Mat& edges,
    cv::Mat& _sobel_x, cv::Mat& _sobel_y,
    int apertureSize, bool L2gradient)
{
    edges = cv::Mat(image.size(), CV_8U);
    _sobel_x = cv::Mat(image.size(), CV_16S);
    _sobel_y = cv::Mat(image.size(), CV_16S);


    cvCanny3(image, edges,
        _sobel_x, _sobel_y,
...
int fitEllipse2(double* S_, double* ellicoeff)
{
    // https://eigen.tuxfamily.org/dox/classEigen_1_1GeneralizedEigenSolver.html#a62f01cd78271efd5e39bcb24e0fe1a58
    ...
    */
    ...
    //cv::Mat C = cv::Mat::zeros(6, 6, CV_64FC1);
    C(0, 2) = 2;
    C(2, 0) = 2;
    C(1, 1) = -1;
    ...
    Eigen::VectorXd alphar = ges.alphas().real();
    Eigen::VectorXd alphai = ges.alphas().imag();
    Eigen::VectorXd beta = ges.betas();
    Eigen::MatrixXd vr = ges.eigenvectors().real().transpose();
    ...
    if (index != -1)
    {
        if (vr(index, 0) <= 0)
        {
            ellicoeff[0] = -vr(index, 0);
            ellicoeff[1] = -vr(index, 1);
            ellicoeff[2] = -vr(index, 2);
            ellicoeff[3] = -vr(index, 3);
            ellicoeff[4] = -vr(index, 4);
            ellicoeff[5] = -vr(index, 5);
        }
        else
        {
            ellicoeff[0] = vr(index, 0);
...
bool calcEllipseParametersAndValidate(double* lines, int line_num, std::vector<std::vector<int>>* groups, int first_group_ind, int second_group_ind, double* fit_matrix1, double* fit_matrix2, image_double angles, double distance_tolerance, unsigned int* group_inliers_num, point5d* ellipara)
{
    ...
    if (fit_matrix2 == NULL || second_group_ind == -1)
    {
        for (int i = 0; i < 36; i++)
        {
            S[i] = fit_matrix1[i];
    ...
    {
        addFitMatrix(fit_matrix1, fit_matrix2, S);
    }
...
void  generateEllipseCandidates(cv::Mat& I,
    ...
    cv::Mat& gradient_vec_out)
{
    ...
    for (int c = 0; c < imgx; c++)
    {
        ...
        {
            data[c + r * imgx] = inputimg[c + r * imgx];
        }
    }
    int n;
    //int new_n;
    ...
    if (pairGroupList != NULL)
    {
        ...
        for (int i = 0; i < candidates_num; ++i)
        {
            candidates_out.at<double>(0, i) = candidates[i * 5 + 0];
            candidates_out.at<double>(1, i) = candidates[i * 5 + 1];
            candidates_out.at<double>(2, i) = candidates[i * 5 + 2];
            candidates_out.at<double>(3, i) = candidates[i * 5 + 3];
            candidates_out.at<double>(4, i) = candidates[i * 5 + 4];
        }

        //memcpy(candidates_out.data,candidates,sizeof(double)*5*candidates_num);
    ...
    edgeimg_out = cv::Mat(imgy, imgx, CV_8UC1);
    ...
    for (int c = 0; c < imgx; c++)
    {
        for (int r = 0; r < imgy; r++)
        {
            ...
            else
            {
                ...
                edgePts.push_back(cv::Point(c, r));
                //------------------------------------------------
    ...
    for (int c = 0; c < imgx; c++)
    {
        for (int r = 0; r < imgy; r++)
        {
            ...
            if (angles->data[addr] != NOTDEF)
            {
                gradient_vec_out.at<double>(0, g_cnt) = cos(angles->data[addr]);
                gradient_vec_out.at<double>(1, g_cnt) = sin(angles->data[addr]);
                g_cnt++;
            }
        }
    }
